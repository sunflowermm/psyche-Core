...
function getTypeAsset(engine, typeCode) {
  ...
  if (engine === 'mbti-oejts' || engine === 'mbti-binary') {
    return assets.mbti?.[key.toUpperCase()] || null
  }
  if (engine === 'cbti') return assets.cbti?.[key] || null
...
export function enrichResult(result, assessment) {
  ...
  if ((engine === 'cbti' || engine === 'sbti' || engine === 'shennong') && result.personality) {
    typeCode = result.personality.code
  }
...
/** @param {ReturnType<typeof enrichResult>} enriched */
export function buildRenderPayload(enriched, assessment, lang = 'zh', hiddenMeta = null) {
  const p = enriched.personality || {}
  const dims = []

  if (enriched.dimensionSummary) {
    for (const [code, info] of Object.entries(enriched.dimensionSummary)) {
      dims.push({ code, label: code, left: info.leftPercent, right: info.rightPercent, preference: info.preference })
    }
  } else if (enriched.dimensions?.length) {
    for (const d of enriched.dimensions) {
      dims.push({
        code: d.code,
        label: d.name || d.code,
        value: d.percentage ?? (d.levelNum == null ? 50 : d.levelNum * 50),
        level: d.level,
        hint: d.explanation || ''
      })
    }
  } else if (enriched.domains) {
    const names = { O: '开放性', C: '尽责性', E: '外向性', A: '宜人性', N: '神经质' }
    for (const [code, info] of Object.entries(enriched.domains)) {
  ...
  return {
    ...
    isSpecial: Boolean(enriched.isSpecial),
    petLore: enriched.petLore || null,
    mode: enriched.mode || null,
    secondaryType: enriched.secondaryType
      ? { code: enriched.secondaryType.code, name: enriched.secondaryType.name || enriched.secondaryType.cn }
      : null,
    matchDetails: (enriched.matchDetails || []).slice(0, 5),
