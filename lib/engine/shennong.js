...
export function score(data, answers, extra = {}) {
  ...
  const counts = Object.fromEntries(data.axes.map((a) => [a, 0]))
  ...
  const dimensions = data.axes.map((a) => {
  ...
  const ranked = data.archetypes.map((arch) => {
    ...
    for (let i = 0; i < data.axes.length; i++) {
      ...
      const target = vectorTarget(arch.vector[i] ?? 1)
      const diff = Math.abs(userPct - target)
      sq += diff * diff
      if (levelNum(userPct) === (arch.vector[i] ?? 1)) exact++
    }
    // RMSE → 相似度：维差约 40pct 仍可到 ~50 分
    const rmse = Math.sqrt(sq / data.axes.length)
