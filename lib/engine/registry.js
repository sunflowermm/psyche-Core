...
function readJson(fileName) {
  ...
  const text = fs.readFileSync(path.join(DATA_DIR, fileName), 'utf8').replace(/^\uFEFF/, '')
  ...
  switch (assessment.engine) {
    ...
    case 'bigfive-ipip50':
      return data.map((item) => ({
        id: String(item.id), type: 'likert', dimension: item.domain,
        text: item.text,
...
export function formatResultText(result, assessment, lang = 'zh') {
  if ((assessment.engine === 'cbti' || assessment.engine === 'shennong') && result.personality) {
